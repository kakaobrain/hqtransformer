...
@torch.no_grad()
def compute_statistics_imagenet_val(resolution=128,
                                    batch_size=500,
    ...
    mu_acts, sigma_acts, mu_acts_recon, sigma_acts_recon = \
...
def compute_activations_from_files(path,
                                   batch_size=500,
                                   inception_model=None,
                                   device=torch.device('cuda'),
                                   ):
    dataset = create_dataset_from_files(path)
    return compute_activations_from_dataset(dataset,
                                            batch_size=batch_size,
                                            inception_model=inception_model,
                                            device=device)
...
def compute_statistics_from_files(path,
    ...
    acts = compute_activations_from_files(path,
                                          batch_size=batch_size,
                                          inception_model=inception_model,
                                          device=device,
                                          )
