...
from hqvae.models.stage2.layers import Block
...
class Transformer1d(nn.Module):
    ...
    def __init__(self,
        ...
        self.blocks = [Block(ctx_len=hparams.ctx_len_img + hparams.ctx_len_txt,
                             ...
                             causal_attn=causal_attn) for i in range(1, hparams.n_layers+1)]
        self.blocks = nn.Sequential(*self.blocks)
    ...
    def forward(self,
                ...
                texts: torch.LongTensor) -> Tuple[torch.FloatTensor, torch.FloatTensor]:
        B, T = images.shape
        _, N = texts.shape
        ...
        pos_texts = torch.arange(N, device=images.device).repeat((B, 1))
        pos_images = torch.arange(T, device=images.device).repeat((B, 1))
